...
import { Calendar, Package, FileText, CheckCircle, Clock, AlertCircle, Users, TrendingUp, Target, Award, Loader2 } from 'lucide-react';
...
export default function TeamDashboard() {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      if (res.ok && data.attendance) {
        // Filter attendance only for the current user
  ...
  const attendanceRate = totalDaysTracked > 0 ? ((presentDays / totalDaysTracked) * 100).toFixed(1) : '0';
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600">Welcome back, {user?.name}! Here are your assignments and attendance.</p>
      </div>
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border">
        <div className="p-6 border-b border-border flex items-center justify-between">
          ...
          <Link href="/team/bookings" className="text-sm text-primary hover:text-primary/80 font-medium">
            View All →
          </Link>
        </div>
        <div className="divide-y divide-border">
          {bookings.length === 0 ? (
            ...
            bookings.slice(0, 5).map((assignment) => (
              <div key={assignment.id} className="p-6 hover:bg-gold/10 transition-colors cursor-pointer">
