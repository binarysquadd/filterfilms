...
import { Search, Check, X, Eye, Loader2, Edit2, Trash2, ChevronDown, Calendar, Box } from 'lucide-react';
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="h-72 text-center align-middle text-sm text-muted-foreground">
                ...
                filteredBookings.map((booking) => {
                  ...
                  return (
                    <tr key={booking.id} className="hover:bg-muted/30 cursor-pointer">
                      ...
                      <td className="p-4">
                        <Button variant="close" size="icon" onClick={() => setSelectedBooking(booking)}>
          ...
          <div className="bg-card rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-elegant animate-scale-in">
            <div className="p-5 border-b border-border flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {!isEditMode && (
                  <>
                    <Button variant="close" size="icon" onClick={() => openEditMode(selectedBooking)}>
                      <Edit2 className="w-4 h-4" />
                    </Button>
                    <Button variant="close" size="icon" onClick={() => openDeleteModal(selectedBooking.id)}>
                      <Trash2 className="w-4 h-4 text-red-500" />
                    </Button>
                  </>
                )}
                <Button variant="close" size="icon" onClick={() => { setSelectedBooking(null); setIsEditMode(false); setEditedBooking(null); }}>
                  <X className="w-5 h-5 " />
                </Button>
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditedBooking({ ...editedBooking, endDate: e.target.value })}
                        className="mt-1"
                      />
                    </div>
                  </div>         
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value => setEditedBooking({ ...editedBooking, status: value as Booking['status'] }))}
                      disabled={loading}                      
                    >
                      <SelectTrigger>
                          ...
                          {team.length === 0 ? (
                            ...
                            team.map(member => {
                              ...
                              return (
                                ...
                                >
                                  <span className={`w-4 h-4 rounded border flex items-center justify-center ${isSelected ? 'bg-primary border-primary text-primary-foreground' : 'border-input'
                                    }`}>
                  ...
                  <div className="flex gap-2 pt-4 justify-end">
                    ...
                    <Button size="sm" onClick={handleSaveEdit} disabled={loading} className="min-w-[140px] flex items-center justify-center">
                      {loading ? (
                        <>
