...
const AdminAttendancePage = () => {
    ...
    const handleMarkAttendance = async (memberId: string, status: 'present' | 'absent' | 'half-day' | 'leave') => {
        try {
            setSaving(true);
            ...
            if (existing) {
                ...
                const response = await fetch(`/api/admin/attendance/${existing.id}`, {
                    ...
                    body: JSON.stringify({
                        status,
                        checkIn: status === 'present' || status === 'half-day' ? (existing.checkIn || '09:00') : undefined,
                        checkOut: status === 'present' ? (existing.checkOut || '18:00') : status === 'half-day' ? '13:00' : undefined,
                    }),
                });
                            ...
                            <tbody className="divide-y divide-border">
                                {filteredMembers.length === 0 ? (
                                    ...
                                    filteredMembers.map((member) => {
                                        const record = attendance.find(a => a.memberId === member.id && a.date === selectedDate);
                                        return (
                                            <tr key={member.id} className="hover:bg-popover transition-colors cursor-pointer">
                                                <td className="p-4">
                                                ...
                                                <td className="p-4">
                                                    {record ? (
                                                        <span className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold ${record.status === 'present' ? 'bg-emerald-100 text-emerald-700' :
                                                ...
                                                <td className="p-4">
                                                    {record?.checkIn ? (
                                                        <input
                                                            ...
                                                            onChange={(e) => handleUpdateTime(record.id, 'checkIn', e.target.value)}
                                                ...
                                                <td className="p-4">
                                                    {record?.checkOut ? (
                                                        <input
                                                            ...
                                                            onChange={(e) => handleUpdateTime(record.id, 'checkOut', e.target.value)}
                                                            className="px-3 py-1.5 border border-border rounded-lg text-sm outline-none bg-card text-muted-foreground"
                                                        />
                                                    ) : (
                                                        <span className="text-muted-foreground">—</span>
                                                ...
                                                <td className="p-4">
                                                    <div className="flex items-center gap-2">
                                                        <button
                                                            ...
                                                            disabled={saving}
                                                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${record?.status === 'present'
                                                        ...
                                                        <button
                                                            ...
                                                            disabled={saving}
                                                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${record?.status === 'absent'
                                                        ...
                                                        <button
                                                            ...
                                                            disabled={saving}
                                                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${record?.status === 'half-day'
                                                        ...
                                                        <button
                                                            ...
                                                            disabled={saving}
                                                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${record?.status === 'leave'
